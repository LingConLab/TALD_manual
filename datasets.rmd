...
For a quick reference on how to collect data, see the [Step-by-step](https://lingconlab.github.io/TALD_manual/steps.html) instructions. The instructions below go into more detail.
...
* **genlang_point** - for our maps showing one datapoint per language we need to choose which datapoint (in case we have multiple points per language) is representative and will thus be showed on this map. For this purpose you need to set the appropriate row in this column to *yes* (no more than one per language), and set all other observations for the same language to *no*; see also [Philosophy - Current approach](https://lingconlab.github.io/TALD_manual/philosophy.html#current_approach). The full list of languages can be found [here](https://lingconlab.github.io/TALD_manual/philosophy.html#list_of_languages)
...
> One value = one row in the dataset. If you have multiple values for one idiom, create two rows for this idiom, each with a unique id. General rule of thumb for the **genlang_point** column is 1 "yes" per language (the Dargwa varieties listed in [Language sample](https://lingconlab.github.io/TALD_manual/philosophy.html#The_language_sample) each count as a separate language), for **map** it is 1 "yes" per idiom.
...
* **form** - what the phoneme / morpheme / construction / lexeme looks like in [Caucasiologist transcription](#transcription)
* **example** - example of how the form is used (if applicable) in [Caucasiologist transcription](#transcription) with morpheme boundaries
