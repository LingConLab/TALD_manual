...
When adding values to the `TITLE` and `BOOKTITLE` fields, you need to surround capittal letters that are not at the beginning of the word with curly brackets, for example `Volume {IV}`. This is necessary so that such cases do not change to `Volume Iv` after rendering.
